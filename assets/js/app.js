var services = {
  ...
  customerservice: {
    ...
    word: "noun",
    phonetics: "/ˈkʌstəmə ˈsəːvɪs/",
    description:
      "As the saying goes, the customer is always right. Here at NyteVision we take that saying with utmost importance. All customer service agents are fully fledged cyber security spcialist who will not only solve all difficulties that you might be having, they take measures to avoid no such difficulty will befall you again. We also have very powerful Machine Learning running in the background of Nytevision which will in turn steer you away from social engineering attacks and similar cyber security issues",
  },
};
...
console.log(
  "%cThis website was built by me (Hasala Heiyanthuduwa) for the INTEROP'20 ICT Competition, held by the Vidura College ICT Society",
  "font-size:12.5px;"
);
...
const openModal = (e) => {
  ...
  modalServiceCard.style.setProperty(
    "--background-image",
    `url('/assets/img/${services[selectedService].image}')`
  );
...
function rotate(direction) {
  ...
  if (direction === "next" && parseInt(currentRotatationValue) < 360) {
    ...
    direction === "before" &&
    parseInt(currentRotatationValue) > -360
  ) {
    let rotationValue = parseInt(currentRotatationValue) - 180;
    card.style.transform = `rotateY(${rotationValue}deg)`;
    tracker(rotationValue);
...
function tracker(rotationValue) {
  ...
  sections = sections.filter((n) => {
    return n !== rotationValue;
  });
  document.getElementById(`tracker_(${rotationValue})`).textContent =
    "radio_button_checked";
  sections.map((trackno) => {
    document.getElementById(`tracker_(${trackno})`).textContent =
